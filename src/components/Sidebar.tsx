...
export default function Sidebar({
  ...
  return (
    <aside 
      id="main-sidebar" 
      className={`${isCollapsed ? 'w-20' : 'w-80'} bg-[#050505] border-r border-white/10 text-slate-100 flex flex-col shrink-0 h-screen sticky top-0 overflow-hidden transition-all duration-300 ease-in-out`}
    >
      {/* Brand Header */}
      {isCollapsed ? (
        <div className="p-4 border-b border-white/10 flex flex-col items-center gap-4 bg-black/40">
          ...
          <button 
            ...
            className="p-1.5 hover:bg-white/10 text-amber-500 hover:text-amber-400 rounded-lg transition"
          ...
          {menuItems.map((item) => {
            ...
            return (
              <button
                ...
                className={`w-full flex items-center ${isCollapsed ? 'justify-center px-1' : 'justify-between px-3'} py-2.5 rounded-lg text-xs font-semibold transition-all duration-200 ${
                  isActive
        ...
        {isCollapsed ? (
          <div className="flex flex-col items-center gap-4">
            <div 
              ...
              className="w-8 h-8 rounded-full bg-amber-500 text-black flex items-center justify-center font-bold text-xs shrink-0 shadow-[0_0_10px_rgba(245,158,11,0.2)] cursor-help"
            >
              {currentUser?.role || 'MD'}
            </div>
            <button 
